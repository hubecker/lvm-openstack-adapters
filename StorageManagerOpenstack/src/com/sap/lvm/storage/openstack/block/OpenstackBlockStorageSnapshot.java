	    	 ...
	    	 //  snapshotState  = snapshot.getState();
	    		...
	    		//   progress = Integer.parseInt(snapshot.getProgress()); //TODO might need to be trimmed if it has % at the end
	    		  ...
	    		  context.snapshotIds.set(i,null);
	    		   ...
	    		   logMessages.add(new StorageLogMessage(IJavaEeLog.SEVERITY_ERROR, "SMI-S", System.currentTimeMillis(), "Failed to Snapshot volume " + context.volumes.get(i).storageVolumeId +". Snapshot process terminated with ERROR") );	
	    	       failed = true;
	    		   break;
	    	   } else {
